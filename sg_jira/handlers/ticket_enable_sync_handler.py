# TODO: Handle the sync checkbox
# See: enable_syncing_handler
...
#
from sg_jira.constants import SHOTGUN_SYNC_IN_JIRA_FIELD
from sg_jira.handlers import SyncHandler


class TicketEnableSyncingHandler(SyncHandler):
    """
    A handler which handles Tickets when "Sync In Jira" (sg_sync_in_jira) checkbox field
    ...
    This is based on the EnableSyncingHandler for Tasks in enable_syncing_handler.py .
    """
    ...
    def __init__(self, syncer, handlers):
        ...
        """
        super(TicketEnableSyncingHandler, self).__init__(syncer)
        if not handlers:
